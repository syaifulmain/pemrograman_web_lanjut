...
{{--            <img src="{{ asset('polinema-bw.png') }}" alt="">--}}
{{--        </td>--}}
        <td width="85%">
...
<table class="border-all">
    ...
    @foreach($stok as $i)
        <tr>
            ...
            <td>{{ $i->barang->barang_kode }}</td>
            <td>{{ $i->barang->barang_nama }}</td>
            <td>{{ $i->user->nama }}</td>
            <td>{{ $i->supplier->supplier_nama }}</td>
            <td>{{ \Carbon\Carbon::parse($i->stok_tanggal)->translatedFormat('d F Y') }}</td>
            <td class="text-right">{{ number_format($i->stok_jumlah, 0, ',', '.') }}</td>
        </tr>
    @endforeach
    </tbody>
</table>
